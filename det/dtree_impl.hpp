    ...
    criterion(criterion)
{
    ...
    density = std::exp(std::log(ratio) - logVolume);
}
    ...
    criterion(criterion)
{
    ...
    ratio = (double) (end - start) / (double) totalPoints;
    density = std::exp(std::log(ratio) - logVolume);
}
...
template<typename MatType, typename TagType>
size_t DTree<MatType, TagType>::Depth() const
{
    if (subtreeLeaves == 1){
        return 1;
    }
...
double DTree<MatType, TagType>::LogNegativeError(const size_t totalPoints) const
{
    ...
    if (criterion == "L2"){
      ...
      err = 2 * std::log((double) (end - start)) -
    ...
    else if (criterion == "NLL"){
      err = std::log((double) (end - start)) -
                   std::log((double) totalPoints);
                                        ...
                                        const size_t mtry) const
{
 ...
 // std::cout << "total dim: " << maxVals.n_elem << endl;
 arma::arma_rng::set_seed_random();
 arma::uvec dim_list = arma::sort(arma::randperm(maxVals.n_elem, mtry));
 // std::cout << dim_list << endl;
 // size_t dim;
  // Loop through each dimension.
#ifdef _WIN32
  #pragma omp parallel for default(shared)
 // for (intmax_t dim = 0; dim < (intmax_t) maxVals.n_elem; ++dim)
    // for (intmax_t k = 0; k < (intmax_t) mtry; ++k)
  for (intmax_t dim : dim_list)
#else
  #pragma omp parallel for default(shared)
 // for (size_t dim = 0; dim < maxVals.n_elem; ++dim)
    // for (size_t k = 0; k < mtry; ++k)
  for (size_t dim : dim_list)
#endif
  {
    // dim = dim_list(k);
//    std::cout << "dim: " << dim << endl;
    ...
    double minDimError;
    if (criterion == "L2"){
        minDimError = std::pow(points, 2.0) / (max - min);
    }
    else if (criterion == "NLL"){
        minDimError = points * (std::log(points) - std::log(max - min));
    }
         ...
         ++i)
    {
      ...
      if ((split - min > 0.0) && (max - split > 0.0))
      {
        ...
        double negLeftError;
        double negRightError;
        if (criterion == "L2"){
            negLeftError = std::pow(position, 2.0) / (split - min);
            negRightError = std::pow(points - position, 2.0) / (max - split);
        }
        else if (criterion == "NLL"){
            negLeftError = position * (std::log(position) - std::log(split - min));
            negRightError = (points - position) * (std::log(points - position) - std::log(max - split));
        }
    ...
    if (criterion == "L2") {
        actualMinDimError = std::log(minDimError)
        - 2 * std::log((double) data.n_cols)
        - volumeWithoutDim;
    }
    else if (criterion == "NLL") {
        actualMinDimError = minDimError
        - points * std::log((double) data.n_cols)
        - points * volumeWithoutDim;
    }
...
#pragma omp critical(DTreeFindUpdate)
    if ((actualMinDimError > minError) && dimSplitFound)
    {
      ...
      if (criterion == "L2") {
          ...
          leftError = dimLeftError
            - opt_position * std::log((double) data.n_cols)
            - opt_position * volumeWithoutDim;

          rightError = dimRightError
            - (points - opt_position) * std::log((double) data.n_cols)
            - (points - opt_position) * volumeWithoutDim;
      }
      splitFound = true;
...
template<typename MatType, typename TagType>
void DTree<MatType, TagType>::ComputeLossReduction(MatType& data,
    ...
    if ((size_t) (end - start) > maxLeafSize){
        ...
        if (FindSplit(data, dim, splitValueTmp, errorL, errorR, minLeafSize, mtry)){
            splitValue = splitValueTmp;
            splitDim = dim;
            leftError = errorL;
            rightError = errorR;
            reduction = leftError + rightError - logNegError;
        }
        else{
            reduction = -DBL_MAX;
        }
...
template<typename MatType, typename TagType>
void DTree<MatType, TagType>::GrowOnce(MatType& data,
...
//  ratio = (double) (end - start) / (double) oldFromNew.n_elem;
  ...
  const size_t splitIndex = SplitData(data, splitDim, splitValue, oldFromNew);
  ...
  maxValsL[splitDim] = splitValue;
  minValsR[splitDim] = splitValue;

  // Recursively grow the children.
  left = new DTree(maxValsL, minValsL, start, splitIndex, oldFromNew.n_elem, leftError, criterion);
  right = new DTree(maxValsR, minValsR, splitIndex, end, oldFromNew.n_elem, rightError, criterion);
...
//      logVolume += std::log(maxVals[i] - minVals[i]);
  ...
  if ((size_t) (end - start) > maxLeafSize)
  {
    ...
    if (FindSplit(data, dim, splitValueTmp, leftError, rightError, minLeafSize, mtry))
    {
      ...
      if (criterion == "L2") {
          ...
          subtreeLeavesLogNegError = left->SubtreeLeavesLogNegError() + right->SubtreeLeavesLogNegError();
      }
    }
    else
    {
  ...
  else
  {
    ...
    const double leftLogVol = std::log(std::exp(logVolume) * leftRatio);
    const double rightLogVol = std::log(std::exp(logVolume) * rightRatio);
    ...
    if (criterion == "L2"){
        leftPow = std::pow((double) (left->End() - left->Start()), 2);
        rightPow = std::pow((double) (right->End() - right->Start()), 2);
        thisPow = std::pow((double) (end - start), 2);
        tmpAlphaSum = leftPow / leftRatio + rightPow / rightRatio - thisPow;
    }
    else if (criterion == "NLL"){
        double leftP = left->End() - left->Start();
        double rightP = right->End() - right->Start();
        leftLoss = leftP * (std::log(leftP) - leftLogVol - std::log((double) data.n_cols));
        rightLoss = rightP * (std::log(rightP) - rightLogVol - std::log((double) data.n_cols));
        thisLoss = ((double) (end - start)) * (std::log((double) (end - start)) - logVolume - std::log((double) data.n_cols));
        tmpAlphaSum = leftLoss + rightLoss - thisLoss;
    }
    ...
    {
        if (criterion == "L2"){
          exponent = 2 * std::log((double) data.n_cols) + logVolume +
            left->AlphaUpper();
          tmpAlphaSum += std::exp(exponent);
        }
        else if (criterion == "NLL"){
          exponent = left->AlphaUpper();
          tmpAlphaSum += exponent;
        }
    }
    ...
    {
        if (criterion == "L2"){
          exponent = 2 * std::log((double) data.n_cols)
           + logVolume
           + right->AlphaUpper();
    ...
    else
    {
        if (criterion == "L2"){
          ...
          gT = std::log(alphaUpper) - std::log((double) (subtreeLeaves - 1));
        }
    }
    ...
    if (gT > oldAlpha)
    {
      ...
      if (criterion == "L2"){
          ...
          tmpAlphaSum = leftPow / leftRatio + rightPow / rightRatio - thisPow;
           ...
           double leftP = left->End() - left->Start();
           double rightP = right->End() - right->Start();
           leftLoss = leftP * (std::log(leftP) - leftLogVol - std::log((double) points));
           rightLoss = rightP * (std::log(rightP) - rightLogVol - std::log((double) points));
           thisLoss = ((double) (end - start)) * (std::log((double) (end - start)) - logVolume - std::log((double) points));
           tmpAlphaSum = leftLoss + rightLoss - thisLoss;
      }
      ...
      {
          if (criterion == "L2"){
            exponent = 2 * std::log((double) points) + logVolume +
              left->AlphaUpper();
            tmpAlphaSum += std::exp(exponent);
          }
          else if (criterion == "NLL"){
      ...
      {
          if (criterion == "L2"){
            exponent = 2 * std::log((double) points)
             + logVolume
             + right->AlphaUpper();
    ...
    const
{
  ...
  while (!nodes.empty())
  {
    ...
    if (criterion == "L2")
        ...
        importances[curNode.SplitDim()] += (-curNode.LogNegError() -
        (-curNode.Left()->LogNegError() -curNode.Right()->LogNegError()));
...
template<typename MatType, typename TagType>
void DTree<MatType, TagType>::SetMask(std::unordered_set<size_t> mask){
    this->mask = mask;
}

template<typename MatType, typename TagType>
void DTree<MatType, TagType>::SetCondMask(std::unordered_set<size_t> condMask){

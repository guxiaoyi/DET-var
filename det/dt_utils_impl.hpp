...
template<typename MatType, typename TagType>
void GetSplits(DTree<MatType, TagType>* dtree,
               std::vector<double>& split_v,
               std::vector<double>& split_d)
{
  if (dtree->SubtreeLeaves() != 1){
    split_v.push_back(dtree->SplitValue());
    split_d.push_back(dtree->SplitDim());

    Get_Splits(dtree->Left(), split_v, split_d);
    Get_Splits(dtree->Right(), split_v, split_d);
  }
  
}

template<typename MatType, typename StatType>
void GetLvlSets(DTree<MatType>* dtree,
                std::vector<double>& densities,
                std::vector<StatType>& upper,
                std::vector<StatType>& lower)
{
    if (dtree->SubtreeLeaves() != 1){
        GetLvlSets(dtree->Left(), densities, upper, lower);
        GetLvlSets(dtree->Right(), densities, upper, lower);
    }
    else{
//        double dens = std::exp(std::log(dtree->Ratio()) - dtree->LogVolume());
        densities.push_back(dtree->Density());
        upper.push_back(dtree->MaxVals());
        lower.push_back(dtree->MinVals());
...
{
  // Initialize the tree.
  DTree<MatType, TagType>* dtree = new DTree<MatType, TagType>(dataset, criterion);
  ...
  arma::vec L2LossConstants(prunedSequence.size());
  L2LossConstants.fill(0.0);
  ...
  for (omp_size_t fold = 0; fold < (omp_size_t) folds; fold++)
  {
    ...
    // Initialize the tree.
    DTree<MatType, TagType> cvDTree(train, criterion);
    ...
    arma::vec cvL2LossConstants(prunedSequence.size());
    cvL2LossConstants.fill(0.0);
    for (size_t i = 0;
         i < ((prunedSequence.size() < 2) ? 0 : prunedSequence.size() - 2); ++i)
    {
        ...
        if (criterion == "L2"){
          ...
          cvRegularizationConstants[i] += 2.0 * cvVal
          // / (double) cvData.n_cols;
          / (double) test.n_cols;
          cvL2LossConstants[i] = std::exp(cvDTree.SubtreeLeavesLogNegError());
        }
          ...
          {
            arma::vec testPoint = test.unsafe_col(j);
            // cout << "log of compute values (1): " << std::log(cvDTree.ComputeValue(testPoint)) << endl;
            current_dens = cvDTree.ComputeValue(testPoint);
            // prevent overflow
            if (current_dens > 1e-50){
               cvVal += std::log(current_dens);
            }
          }

          cvRegularizationConstants[i] += cvVal / (double) test.n_cols;
        }
    ...
    for (size_t i = 0; i < test.n_cols; ++i)
    {
      ...
      if (criterion == "L2"){
          ...
          double current_dens = cvDTree.ComputeValue(testPoint);
          // prevent overflow
          if (current_dens > 1e-50){
             cvVal += std::log(current_dens);
          }
    ...
    if (prunedSequence.size() > 2){
      if (criterion == "L2"){
        ...
        // / (double) cvData.n_cols;
        / (double) test.n_cols;
        cvL2LossConstants[prunedSequence.size() - 2] = std::exp(cvDTree.SubtreeLeavesLogNegError());
      }
      else if (criterion == "NLL"){
        cvRegularizationConstants[prunedSequence.size() - 2] += cvVal / (double) test.n_cols;
      }
    }

    #pragma omp critical(DTreeCVUpdate)
    regularizationConstants += cvRegularizationConstants;
    L2LossConstants += cvL2LossConstants;
  }
  ...
  for (size_t i = 0; i < prunedSequence.size() - 1; ++i)
  {
    ...
    if (criterion == "L2"){
        thisError = (long double) -L2LossConstants[i] +
         (long double) regularizationConstants[i];

    }
    else if (criterion == "NLL"){
        thisError = (long double) regularizationConstants[i];
    }
  ...
  delete dtree;
  dtree = new DTree<MatType, TagType>(dataset, criterion);
